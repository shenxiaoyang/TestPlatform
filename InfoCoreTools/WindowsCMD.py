...
def collect_server_log(ip, username, password, current_time):
    run(r"net use \\{} {} /user:{} 2>nul".format(ip, password, username),shell=True)
    ...
    run(r'xcopy "{}\Repository\*"  "{}\OSN_log\" /y 2>nul'.format(remoteOSNSolutionLog, localLogPath),shell=True)
    run(r'xcopy "{}\*.log" "{}\OSN_log\" /y 2>nul'.format(remoteOSNSolutionLog, localLogPath),shell=True)
    run(r'xcopy "{}\*.cfg" "{}\OSN_log\" /y 2>nul'.format(remoteOSNSolutionLog, localLogPath),shell=True)
    run(r'xcopy "{}\*.txt" "{}\OSN_log\" /y 2>nul'.format(remoteOSNSolutionLog, localLogPath),shell=True)
    run(r'xcopy "{}\*.etl" "{}\OSN_log\" /y 2>nul'.format(remoteOSNSolutionLog, localLogPath),shell=True)
    remoteWindowsDump = r"\\{}\c$\Windows".format(ip)
    run(r'xcopy "{}\*.dmp" "{}\dump\" /y 2>nul'.format(remoteWindowsDump, localLogPath),shell=True)
    remoteDebugLog = r'{}*.log'.format(SERVER_TOOL_PATH).replace(r'C:', r'\\{}\c$'.format(ip))
    run(r'xcopy "{}" "{}\debugview\" /y 2>nul'.format(remoteDebugLog, localLogPath),shell=True)
    run(r"net use \\{} /d /y".format(ip),shell=True)