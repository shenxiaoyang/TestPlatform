...
class RefreshStateThread(threading.Thread):
    ...
    def run(self):
        while 1:
            mutex_lock.acquire()
            logging.debug('定时刷新状态')
            flag = False    #状态变更标记，Ture说明状态已经变更
...
class MainWindows(QMainWindow):   #重载主窗体类，继承QtWidgets.QMainWindow
    def __init__(self, parent=None):    #主窗体构造函数
        ...
        self.start_refresh_state_thread()
    ...
    def connect_all_signal_slot(self):
        ...
        self.action_collect_logs.triggered.connect(lambda:self.triggered_action_collect_logs(WindowsCMD.getCurrentDatetimeString()))
    ...
    def init_ui(self):
        ...
        self.action_auto_service_start_on = QAction(self.rightMenu) #创建菜单项[开启服务自启]
        self.action_auto_service_start_off = QAction(self.rightMenu) #创建菜单项[关闭服务自启]
        ...
        self.rightMenu.addAction(self.action_auto_service_start_on)
        self.rightMenu.addAction(self.action_auto_service_start_off)
        ...
        for group_name in group_dict:
            ...
            for server_ip in group_dict[group_name].server_dict:
                ...
                if ipmi_state_dict[server_ip] == '':  #设置IPMI状态
                    server_item.setText(2, '未配置')
                elif ipmi_state_dict[server_ip] == 1:
                    server_item.setText(2, '电源开启')
                else:
    ...
    def retranslateUi(self):
        ...
        self.action_auto_service_start_on.setText('开启服务自启')
        self.action_auto_service_start_off.setText('关闭服务自启')
